...
@interface TokenizerDomjsTests : XCTestCase
...
- (void)test00
{
    ...
    for (NSString *state in @[@""]) {
        ...
        XCTAssertEqualObjects(tokenizer.allObjects, expectedTokens, @"%@", @"CR in bogus comment state");
    }
}
...
- (void)test01
{
    ...
    for (NSString *state in @[@""]) {
        ...
        tokenizer.state = StateNamed(state);
        XCTAssertEqualObjects(tokenizer.allObjects, expectedTokens, @"%@", @"CRLF in bogus comment state");
    }
}
...
- (void)test02
{
    ...
    for (NSString *state in @[@"RCDATA state", @"RAWTEXT state"]) {
        ...
        tokenizer.state = StateNamed(state);
        XCTAssertEqualObjects(tokenizer.allObjects, expectedTokens, @"%@", @"NUL in RCDATA and RAWTEXT");
    }
}
...
- (void)test03
{
    ...
    for (NSString *state in @[@""]) {
        ...
        tokenizer.state = StateNamed(state);
        XCTAssertEqualObjects(tokenizer.allObjects, expectedTokens, @"%@", @"leading U+FEFF must pass through");
    }
}
...
- (void)test04
{
    ...
    for (NSString *state in @[@"RCDATA state"]) {
        ...
        tokenizer.state = StateNamed(state);
        XCTAssertEqualObjects(tokenizer.allObjects, expectedTokens, @"%@", @"Non BMP-charref in in RCDATA");
    }
}
...
- (void)test05
{
    ...
    for (NSString *state in @[@"RCDATA state"]) {
        ...
        tokenizer.state = StateNamed(state);
        XCTAssertEqualObjects(tokenizer.allObjects, expectedTokens, @"%@", @"Bad charref in in RCDATA");
    }
}
...
- (void)test06
{
    ...
    for (NSString *state in @[@"RCDATA state", @"RAWTEXT state"]) {
        ...
        XCTAssertEqualObjects(tokenizer.allObjects, expectedTokens, @"%@", @"lowercase endtags in RCDATA and RAWTEXT");
    }
}
...
- (void)test07
{
    ...
    for (NSString *state in @[@"RCDATA state", @"RAWTEXT state"]) {
        ...
        tokenizer.state = StateNamed(state);
        [tokenizer setLastStartTag:@"xmp"];
        XCTAssertEqualObjects(tokenizer.allObjects, expectedTokens, @"%@", @"bad endtag in RCDATA and RAWTEXT");
    }
}
...
- (void)test11
{
    ...
    for (NSString *state in @[@""]) {
        ...
        tokenizer.state = StateNamed(state);
        XCTAssertEqualObjects(tokenizer.allObjects, expectedTokens, @"%@", @"Non BMP-charref in attribute");
    }
}
...
- (void)test12
{
    ...
    for (NSString *state in @[@""]) {
        HTMLTokenizer *tokenizer = [[HTMLTokenizer alloc] initWithString:[NSString stringWithFormat:@"<!----!%C-->", (unichar)0x0000]];
        tokenizer.state = StateNamed(state);
        XCTAssertEqualObjects(tokenizer.allObjects, expectedTokens, @"%@", @"--!NUL in comment ");
    }
}
...
- (void)test13
{
    ...
    for (NSString *state in @[@""]) {
        ...
        tokenizer.state = StateNamed(state);
        XCTAssertEqualObjects(tokenizer.allObjects, expectedTokens, @"%@", @"space EOF after doctype ");
    }
}
