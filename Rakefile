...
namespace :gentests do
  desc "Translate html5lib tokenizer tests to SenTestingKit"
  task :tokenizer
...
task :gentests => [:'gentests:tokenizer', :'gentests:tree_construction']
...
END
        ...
        objc << <<-END
        XCTAssertEqualObjects(tokenizer.allObjects, expectedTokens, @"%@", #{description});
    }
}
END
      end
  ...
  task :'gentests:tokenizer' => [objcpath]
end
