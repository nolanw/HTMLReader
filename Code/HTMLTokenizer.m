...
{
    // Binary search to find the right area.
    static int (^comparator)(const void *, const void *) = ^int(const void *voidKey, const void *voidItem) {
        ...
        unichar firstKey = [key characterAtIndex:0];
        unichar firstItem = [item->name characterAtIndex:0];
        if (firstKey < firstItem) {
            return -1;
        } else if (firstKey > firstItem) {
            return 1;
        } else {
            return 0;
        }
    };
    const size_t count = sizeof(NamedReferences) / sizeof(NamedReferences[0]);
    void *voidSearch = (__bridge void *)search;
    NamedReferenceTable *prefixItem = bsearch_b(voidSearch, NamedReferences, count, sizeof(NamedReferenceTable), comparator);
    if (!prefixItem) return nil;
    
    // Then search nearby for the longest prefix.
    unichar first = [search characterAtIndex:0];
    NamedReferenceTable *longestPrefixItem = &(NamedReferenceTable){ .name = @"" };
    for (NamedReferenceTable *item = prefixItem; item >= NamedReferences; item--) {
        if (![item->name characterAtIndex:0] == first) break;
    ...
    if (!longestPrefixItem->characters) {
        return nil;
    } else {
